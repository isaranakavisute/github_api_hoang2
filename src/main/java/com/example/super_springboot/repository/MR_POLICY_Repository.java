...
public interface MR_POLICY_Repository extends JpaRepository<MrPolicy, Integer> {
    ...
    @Query(value = "SELECT POCY_OID,POHO_OID,POCY_NO,LMG_NO,EXP_DATE FROM MR_POLICY where POCY_OID=:POCY_OID", nativeQuery = true)
    Collection<MrPolicy> get_policy_no(Long POCY_OID);

    @Query(value = """
        SELECT POCY_OID,POHO_OID,POCY_NO,LMG_NO,EXP_DATE
        FROM MR_POLICY
